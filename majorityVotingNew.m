function[results,missclassfied] = majorityVotingNew(xTrain,yTrain,ids,pca_flag)
...
%     trainIdx =~testIdx;
    trainIdx =find(ids~=i);
%%%%%%%%%%%%%%
%     c = cvpartition(yTrain,'Leaveout');
%     opts = struct('Optimizer','bayesopt','ShowPlots',true,'CVPartition',c,...
%     'AcquisitionFunctionName','expected-improvement-plus');
% svmmod = fitcsvm(xTrain, ...
% yTrain_log,'KernelFunction','rbf',...
...
yTrain(trainIdx), 'Bag', 300, 'Tree', 'Type', 'Classification');
...
t = templateTree('surrogate','all');
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%    AdaboostModel = fitcensemble(xTrain(trainIdx,:),yTrain(trainIdx),'method','Bag','NumLearningCycles',50,'Learners','Tree');
%      AdaboostModel = fitcensemble(xTrain(trainIdx,:),yTrain(trainIdx),'Method','RobustBoost', ...
%     'NumLearningCycles',300,'Learners','Tree','RobustErrorGoal',0.01, ...
%     'RobustMaxMargin',1);
% cost.ClassificationCosts = [0 5; 1 0];
 AdaboostModel =fitcensemble(xTrain(trainIdx,:),yTrain(trainIdx),'Method','GentleBoost', ...
    'NumLearningCycles',150,'Learners',t,'LearnRate',0.1,'Cost',[0 1; 5 0]);




    [AdabPreds, AdabScores] = predict(AdaboostModel, ...
xTrain(testIdx,:));
    finalPreds3 = [finalPreds3; AdabPreds];
    
    finalScores3 = [finalScores3; AdabScores];
...
[ SE_svm,SP_svm,Acc_svm,Prec_svm,Kidx_svm] = classperf( finalPreds1,yTrain )
[ SE_bag,SP_bag,Acc_bag,Prec_bag,Kidx_bag ] = classperf( finalPreds2,yTrain )
%[ SE_lda,SP_lda,Acc_lda,Prec_lda ]  = classperf( finalPreds3,yTrain )
 [ SE_adab,SP_adab,Acc_adab,Prec_adab,Kidx_adab ] = classperf( finalPreds3,yTrain)

 [ SE_mj,SP_mj,Acc_mj,Prec_mj,Kidx_mj ]  = classperf( majorityVoteOutput,yTrain )
...
% modelAccuracy = cp.CorrectRate; 
 fprintf('RuBoost model accuracy = %0.3f\n', Acc_adab); 
...
[ SE_mjs,SP_mjs,Acc_mjs,Prec_mjs,Kidx_mjs ]  = classperf( majorityVoteArgmax,yTrain )
...
[Xlda, Ylda, ~, AUClda] = perfcurve(yT, finalScores3(:,2), 1);  
 plot(Xlda,Ylda); legend(['ROC RuBoost, AUC: ',num2str(AUClda)]);
 
 fprintf('RuBoost model AUC = %0.3f\n', AUClda); 

%[Xtree, Ytree, ~, AUCtree] = perfcurve(yT, majorityVoteArgmax, 1);  
...
[X, Y, ~, AUChard] = perfcurve(yT, majorityVoteOutput, 1);  
fprintf('Majority voting (hard) model AUC = %0.3f\n', AUChard); 
plot(X, Y); hold on;

[Xmjs, Ymjs, ~, AUCmjs] = perfcurve(yT, majorityVoteArgmax, 1);  
 plot(Xmjs,Ymjs); legend(['ROC decision tree, AUC: ',num2str(AUCmjs)]);
 
fprintf('Majority voting (soft) model AUC = %0.3f\n', AUCmjs); 
...
set(gca,'FontWeight','bold','FontSize',10,'LineWidth',1);
 legend(['SVM:',num2str(AUCsvm)],['Random Forests:',num2str(AUCbag)],['RobustBoost:',num2str(AUClda)],['MVhard:',num2str(AUChard)],['MVsoft:',num2str(AUCmjs)]);
...
results = table([SE_svm;SE_bag;SE_adab;SE_mj;SE_mjs],[SP_svm;SP_bag;SP_adab;SP_mj;SP_mjs],[Prec_svm;Prec_bag;Prec_adab;Prec_mj;Prec_mjs],[Kidx_svm;Kidx_bag;Kidx_adab;Kidx_mj;Kidx_mjs],[Acc_svm;Acc_bag;Acc_adab;Acc_mj;Acc_mjs],[AUCsvm;AUCbag;AUClda;AUChard;AUCmjs],...
'VariableNames',{'Sensitivity' 'Specificity' 'Precision' 'Kappaindex' 'Accuracy' 'AUC'},...
'RowNames',{'SVM' 'RF' 'RUBoost' 'MVhard' 'MVsoft'})
